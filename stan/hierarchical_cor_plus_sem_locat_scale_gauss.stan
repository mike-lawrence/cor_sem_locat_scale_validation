...
model{
	...
	if(prior_informed){
		// locat_intercept_mean ~ weibull(2,1) ; // use this when using zero-lower-bound
		locat_intercept_mean ~ std_normal() ; // use this when when unbound
		locat_intercept_sd ~ weibull(2,1) ;
		locat_coef_mean ~ std_normal() ;
		locat_coef_sd ~ weibull(2,1) ;

		locat_cholfaccorr ~ lkj_corr_cholesky(1) ;
		to_vector(locat_icoef_indiv_helper) ~ std_normal() ;

		// scale_intercept_mean ~ weibull(2,1) ; // use this when using zero-lower-bound
		scale_intercept_mean ~ std_normal() ; // use this when when unbound
		scale_intercept_sd ~ weibull(2,1) ;
		scale_coef_mean ~ std_normal() ;
		scale_coef_sd ~ weibull(2,1) ;

		// locat_scale_cors ~ uniform(-1,1) ; //commented-out bc implied by bounds
		to_vector(scale_icoef_indiv_unique) ~ std_normal() ;
